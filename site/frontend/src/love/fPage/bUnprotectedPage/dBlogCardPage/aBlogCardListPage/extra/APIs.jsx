...
const APIs = {
  ...
  ListAPI: (Redux, ReduxUltimate) => {
    ...
    API.GlobalAPI.UnprotectedAPI.BlogCardAPI.ListAPI()
    .then(response => {
      ...
      if (serverResponse.success === true) {
        Redux.dispatch({ type: Redux.action.ReceivedObject, payload: {
					...
					BlogCardList: serverResponse.list.cBlogCards?.map(each => {
            return {
              ...each,
              toRetrieve: `${FinalRouteName.GlobalRoute.BlogCardRetrieveRoute}/${each._id}`,
            }
          })
				} })
      }
